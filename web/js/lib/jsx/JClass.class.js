...
var JClass = {
	...
	create: function(cls,prt,obj) {
		...
		// on ajoute des prori�t�s et m�thodes communes � tous les objets
	...
	import: function(cls) {
		...
		if(this.loadedClasses.indexOf(file)==-1)
		{
			...
			this.include(file);
			//alert(file);
			if(_class=='undefined') throw("Error: JClass.import: l'importation de "+cls+" a �chou�");
			else
			{
	...
	_import : function(_class,cls,file)
	{
		...
		else throw "Error: JClass.import: l'importation de "+cls+" a �chou�";
	},
	...
	copy: function(src,dst) {
		...
		// on cr�e la destination si inexistante
		try
		{	
			var clss = dst.split('.');
		...
		catch(e){throw "Error: JClass.copy : impossible de cr�er la destination "+dst;}
	},
	...
	/*
	 * A partir du nom d'une classe r�cup�re l'objet class correspondant. ex: 'mon.chemin.de.classe'
	 * @param cls: String
	 */ 
	get: function(src) {
		var p;var oSrc;
	...
	callMethod: function(cls,mthd,param) {
		...
		else throw "Error: JClass.callMethod: la m�thode "+mthd+" n'existe pas pour la classe "+k;	
	},
	
	/*
	 * A partir du nom d'une classe r�cup�re l'objet module corrspondant
	 * @param cls: String
	 */ 
